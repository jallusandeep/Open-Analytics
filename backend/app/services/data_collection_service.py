...
def get_data_collection_summary_service():
    ...
    try:
        ...
        active_run = conn.execute("""
            ...
            FROM upstox_sync_runs
            WHERE status IN ('running', 'cancel_requested')
              AND sync_type = 'upstox_current_instruments'
            ORDER BY started_at DESC
            LIMIT 1;
        """).fetchone()
        ...
        return {
            ...
            "expired_last_sync_at": None,
            "expired_duration_seconds": None,
            "equity_last_sync_at": None,
            "equity_duration_seconds": None,
            "ohlcv_daily_last_sync_at": None,
            "ohlcv_daily_duration_seconds": None,
